...
class FeasibilityFilter:
    ...
    def filter(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, FeasibilityReport]:
        ...
        for idx, row in df.iterrows():
            reason, warnings = self._check_row(row)
            if reason is None:
    ...
    def _check_row(self, row: pd.Series) -> Any:
        ...
        try:
            ...
            warnings = list(getattr(params, "feasibility_warnings", []))
            if getattr(params, "requested_body_holes", 0) != getattr(params, "realized_body_holes", 0):
                warnings.append("requires_rebalance")
            ...
            elif "Distal partition plane infeasible" in msg:
                return "Distal partition infeasible", []
            else:
