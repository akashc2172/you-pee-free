...
class COMSOLResult:
    ...
    q_sh_prox: Optional[float] = None
    q_sh_mid: Optional[float] = None
    q_sh_dist: Optional[float] = None
    q_sh_total_signed: Optional[float] = None
    q_sh_total_abs: Optional[float] = None
    hole_uniformity_cv_grouped: Optional[float] = None
    hole_uniformity_maxmin_grouped: Optional[float] = None
    fraction_partition_lumen: Optional[float] = None
    fraction_partition_holes: Optional[float] = None
    wss_max: Optional[float] = None
    wss_p95_global: Optional[float] = None
    wss_p99_global: Optional[float] = None
    ...
    per_hole_flux_csv_found: bool = False
    per_hole_flux_count: int = 0
...
class ResultParser:
    ...
    def parse_run(self, run_dir: Path, run_id: str) -> COMSOLResult:
        ...
        # Optional: look for per-hole shaft flux CSV
        shaft_flux_csv = run_dir / f"{run_id}_shaft_hole_flux.csv"
        if shaft_flux_csv.exists():
            try:
                self._parse_shaft_hole_flux(shaft_flux_csv, result)
    ...
    def _parse_metrics(self, file_path: Path, result: COMSOLResult) -> None:
        ...
        result.q_sh_prox = maybe_float(*RESULT_COLUMN_ALIASES["q_sh_prox"])
        result.q_sh_mid = maybe_float(*RESULT_COLUMN_ALIASES["q_sh_mid"])
        result.q_sh_dist = maybe_float(*RESULT_COLUMN_ALIASES["q_sh_dist"])
        result.wss_max = maybe_float(*RESULT_COLUMN_ALIASES["wss_max"])
        result.wss_p95_global = maybe_float(*RESULT_COLUMN_ALIASES["wss_p95_global"])
        result.wss_p99_global = maybe_float(*RESULT_COLUMN_ALIASES["wss_p99_global"])
    ...
    @staticmethod
    def _parse_shaft_hole_flux(file_path: Path, result: COMSOLResult) -> None:
        df = pd.read_csv(file_path, comment="%")
        if df.empty:
            return
        result.per_hole_flux_csv_found = True
        result.per_hole_flux_count = len(df)
    ...
    def _finalize_metrics(self, result: COMSOLResult) -> None:
        ...
        hole_values = [result.q_sh_prox, result.q_sh_mid, result.q_sh_dist]
        finite_hole_values = [float(value) for value in hole_values if self._is_finite(value)]
        if finite_hole_values:
            result.q_sh_total_signed = sum(finite_hole_values)
            abs_hole_values = [abs(value) for value in finite_hole_values]
            result.q_sh_total_abs = sum(abs_hole_values)

            if len(abs_hole_values) == 3:
                mean_abs = sum(abs_hole_values) / 3.0
                if mean_abs > 0.0:
                    variance = sum((value - mean_abs) ** 2 for value in abs_hole_values) / 3.0
                    result.hole_uniformity_cv_grouped = math.sqrt(variance) / mean_abs
                    min_abs = min(abs_hole_values)
                    if min_abs > 0.0:
                        result.hole_uniformity_maxmin_grouped = max(abs_hole_values) / min_abs

            if self._is_finite(result.q_out):
                q_out_abs = abs(float(result.q_out))
                denom = q_out_abs + float(result.q_sh_total_abs)
                if denom > 0.0:
                    result.fraction_partition_lumen = q_out_abs / denom
                    result.fraction_partition_holes = float(result.q_sh_total_abs) / denom
