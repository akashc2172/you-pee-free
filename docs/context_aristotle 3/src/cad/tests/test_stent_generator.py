...
class TestStentGenerator:
    ...
    def test_export_step(self, tmp_path: Path):
        ...
        assert out_path.with_suffix(".holes.json").exists()
        assert out_path.with_suffix(".meters.json").exists()
    ...
    def test_hole_metadata_sidecar_has_canonical_frame_and_axial_order(self, tmp_path: Path):
        params = StentParameters(n_prox=2, n_mid=3, n_dist=2)
        gen = StentGenerator(params)
        out_path = tmp_path / "design_0000.step"
        gen.export_step(out_path)

        metadata = json.loads(out_path.with_suffix(".holes.json").read_text())

        assert metadata["schema_version"] == "hole_metadata_sidecar_v2"
        assert metadata["design_id"] == "design_0000"
        assert metadata["frame_definition"]["body_axis"] == "+X"
        assert metadata["frame_definition"]["units"] == "mm"
        assert metadata["export_body_start_x_mm"] == pytest.approx(0.0, abs=1e-6)
        assert len(metadata["holes"]) == params.realized_total_hole_count

        axial_order = metadata["axial_order"]
        axial_xs = [entry["axial_x_mm"] for entry in axial_order]
        assert axial_xs == sorted(axial_xs)
        assert [entry["axial_rank"] for entry in axial_order] == list(range(len(axial_order)))

        for hole in metadata["holes"]:
            normal = hole["normal"]
            norm_mag = sum(value * value for value in normal) ** 0.5
            assert norm_mag == pytest.approx(1.0, abs=1e-6)
            assert hole["axial_x_mm"] == pytest.approx(hole["center_mm"][0], abs=1e-6)

    def test_hole_metadata_preserves_shaft_and_coil_groupings(self, tmp_path: Path):
        params = StentParameters()
        gen = StentGenerator(params)
        out_path = tmp_path / "grouped.step"
        gen.export_step(out_path)

        metadata = json.loads(out_path.with_suffix(".holes.json").read_text())
        holes = metadata["holes"]
        shaft_count = sum(1 for hole in holes if hole["type"] == "shaft")
        coil_count = sum(1 for hole in holes if hole["type"] == "coil")

        assert shaft_count == params.realized_body_holes
        assert coil_count == params.realized_coil_hole_count
        assert metadata["selection_helpers"]["typed_region_selection_boxes"]["prox"]["shaft"] is not None
        assert metadata["selection_helpers"]["typed_region_selection_boxes"]["mid"]["shaft"] is not None

    def test_measurement_surface_sidecar_has_required_features(self, tmp_path: Path):
        params = StentParameters(unroofed_length=12.0)
        gen = StentGenerator(params)
        out_path = tmp_path / "design_0000.step"
        gen.export_step(out_path)

        metadata = json.loads(out_path.with_suffix(".meters.json").read_text())
        features = metadata["features"]
        feature_ids = {feature["feature_id"] for feature in features}

        assert metadata["schema_version"] == "measurement_surface_sidecar_v1"
        assert metadata["design_id"] == "design_0000"
        assert "cap_hole_shaft_prox_000" in feature_ids
        assert "sec_distal_lumen" in feature_ids
        assert "sec_distal_annulus" in feature_ids
        assert "sec_inlet_ref" in feature_ids
        assert "sec_outlet_ref" in feature_ids
        assert "patch_unroof_1" in feature_ids

    def test_distal_coil_holes_span_both_sides_of_loop_after_canonicalization(self):
        """Distal coil metadata should not collapse to one inward-facing phase around the shaft."""
        params = StentParameters(stent_length=220)
        gen = StentGenerator(params)
        gen.generate()

        distal_coil_entries = [
            entry for entry in params.export_hole_entries
            if entry["type"] == "coil" and entry["region"] == "dist"
        ]

        assert len(distal_coil_entries) == len(params.coil_hole_params)

        ys = [float(entry["center_vec"].Y) for entry in distal_coil_entries]
        assert min(ys) < 0.0
        assert max(ys) > 0.0
