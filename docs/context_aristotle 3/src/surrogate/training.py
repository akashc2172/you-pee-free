...
class GPTrainer:
    ...
    def train(self, 
        ...
        Returns:
            (model, metrics)
        """
        # Enforce schema-driven outputs: y must already be the Tier-1 transformed set.
        expected = tier1_target_columns(include_optional=False, schema=SCHEMA_V1)
        if list(y.columns) != expected:
            raise ValueError(
                "GPTrainer.train requires Tier-1 transformed outputs only. "
                f"expected={expected}, got={list(y.columns)}"
            )

        model = GPModel(input_dim=X.shape[1], outcome_dim=y.shape[1])
        model.fit(X, y, x_columns=list(X.columns), y_columns=list(y.columns))
    ...
    def cross_validate(self, 
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
            ...
            # Train
            model = GPModel(input_dim=X.shape[1], outcome_dim=y.shape[1])
            model.fit(X_train, y_train, x_columns=list(X_train.columns), y_columns=list(y_train.columns))
