...
- [`docs/comsol_hole_selection_setup.md`](/Users/akashc/masters/docs/comsol_hole_selection_setup.md)
...
- Use `scripts/print_comsol_template_values.py` to get the current manual template reference values and body-frame metadata for the design you are templating.
...
- Current manually validated cylinder placements: kidney `(r=18, h=31, x=-25)`, ureter `(r=4, x=0, height=body span)`, bladder `(r=18, h=31, x=264)`.
- Current manually validated cap boxes: inlet `x=[-25.5,-24.5]`, outlet `x=[294.5,295.5]`, both with `y,z=[-18,18]`.
...
Recommended baseline scientific-output selections:
- `sel_holes_prox`
- `sel_holes_mid`
- `sel_holes_dist`
- `sel_wall_global`
...
- `q_sh_prox`: `q_sh_prox`, `q_sh_k`, `proximal_side_hole_flow`
- `q_sh_mid`: `q_sh_mid`, `q_sh_u`, `middle_side_hole_flow`
- `q_sh_dist`: `q_sh_dist`, `q_sh_b`, `distal_side_hole_flow`
- `wss_max`: `wss_max`, `max_wall_shear_stress`, `spf_sr_max`
- `wss_p95_global`: `wss_p95_global`, `wss_p95`, `wall_shear_stress_p95`
- `wss_p99_global`: `wss_p99_global`, `wss_p99`, `wall_shear_stress_p99`
...
Derived downstream from parsed CSV values:
- `q_sh_total_signed`
- `q_sh_total_abs`
...
## Continuation solver note

For the current difficult stationary templates, continuation should be treated as the default solver strategy rather than an emergency fallback.

Current successful ramp path:
- define `p_ramp`
- set inlet pressure to `p_ramp*490[Pa]`
- auxiliary sweep: `0.1 0.5 1.0`

Keep this note with the template because it is solver knowledge that needs to survive beyond the current COMSOL session/logs.
