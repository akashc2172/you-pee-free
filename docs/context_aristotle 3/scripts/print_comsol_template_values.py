...
def _build_template_values(stent_params: StentParameters, reference_cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    manual_cfg = reference_cfg.get("manual_validated_template_mm", {})
    ...
    manual_cylinders = manual_cfg.get("cylinders", {})
    kidney_manual = manual_cylinders.get("kidney", {})
    ureter_manual = manual_cylinders.get("ureter", {})
    bladder_manual = manual_cylinders.get("bladder", {})

    kidney_x = float(kidney_manual.get("x_mm", body_start_x - kidney_length))
    ureter_x = float(ureter_manual.get("x_mm", body_start_x))
    bladder_x = float(bladder_manual.get("x_mm", body_end_x))
    kidney_y = float(kidney_manual.get("y_mm", body_center_y))
    kidney_z = float(kidney_manual.get("z_mm", body_center_z))
    ureter_y = float(ureter_manual.get("y_mm", body_center_y))
    ureter_z = float(ureter_manual.get("z_mm", body_center_z))
    bladder_y = float(bladder_manual.get("y_mm", body_center_y))
    bladder_z = float(bladder_manual.get("z_mm", body_center_z))
    kidney_radius = float(kidney_manual.get("radius_mm", kidney_radius))
    kidney_length = float(kidney_manual.get("height_mm", kidney_length))
    ureter_radius = float(ureter_manual.get("radius_mm", ureter_radius))
    bladder_radius = float(bladder_manual.get("radius_mm", bladder_radius))
    bladder_length = float(bladder_manual.get("height_mm", bladder_length))

    inlet_half = inlet_thickness / 2.0
    outlet_half = outlet_thickness / 2.0
    manual_caps = box_cfg.get("manual_validated_caps", {})
    ...
    values = {
        ...
        "selection_boxes": {
            "inlet": {
                "x_range_mm": manual_caps.get("inlet_x_range_mm", [kidney_x - inlet_half, kidney_x + inlet_half]),
                "y_range_mm": manual_caps.get("inlet_y_range_mm", [kidney_y - kidney_radius, kidney_y + kidney_radius]),
                "z_range_mm": manual_caps.get("inlet_z_range_mm", [kidney_z - kidney_radius, kidney_z + kidney_radius]),
            },
            "outlet": {
                "x_range_mm": manual_caps.get(
                    "outlet_x_range_mm",
                    [bladder_x + bladder_length - outlet_half, bladder_x + bladder_length + outlet_half],
                ),
                "y_range_mm": manual_caps.get("outlet_y_range_mm", [bladder_y - bladder_radius, bladder_y + bladder_radius]),
                "z_range_mm": manual_caps.get("outlet_z_range_mm", [bladder_z - bladder_radius, bladder_z + bladder_radius]),
            },
            "coil_zone": {
